...
function getExpectedResult(rawData) {
    ...
    for (let i = 0; i < rawData.instructions.length; i++) {
        let instruction = rawData.instructions[i];
        if(stacks[instruction.from - 1].length < instruction.num) {
            error = "tried to move more elements than available on stack! instruction: " + JSON.stringify(instruction);
        }
        let moveSlice = stacks[instruction.from - 1].slice(-instruction.num);
        stacks[instruction.from - 1] = stacks[instruction.from - 1].slice(0, -instruction.num);
        stacks[instruction.to - 1] = stacks[instruction.to - 1].concat(moveSlice);
    }
