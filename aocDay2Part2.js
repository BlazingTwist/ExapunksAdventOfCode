...
function getTitle() {
    return "Day 2.2 : Rock Paper Scissors";
}
...
function getDescription() {
    return ""
        ...
        + "The only difference is the meaning of X,Y,Z. It is now [X = Lose, Y = Draw, Z = Win]. In case you forgot the old rules:\n"
...
function getExpectedResult(data) {
    ...
    return dataPairs
        .map(function (pair) {
            let outcomeIndex = xyzToIndex(pair[1]);
            let handScore = handScores[outcomeIndex][abcToIndex(pair[0])];
            return (outcomeIndex * 3) + handScore;
        })
        .reduce(sum, 0);
}
