...
function getTitle() {
    return "Day 1.2 : Calorie Counting";
}
...
function getDescription() {
    return ""
        + "Find the problem description here https://adventofcode.com/2022/day/1\n"
        ...
        + "Did you notice that 1.1 used the actual AOC input on testRun 5? I would have told you sooner, but this text box is really small."
        ;
}

function initializeTestRun(testRun) {
    let deckHost = createHost("Deck", 5, -1, 5, 5);
    let bookKeeperHost = createHost("Bookkeeper", 12, -1, 3, 2);
    let captainHost = createHost("Captain", 12, 2, 3, 2);
    let captainTerminal = createWindow("Captain's Terminal", 80, 3, 40, 2);
    let captainGoal = requireCustomGoal("Report sum of max 3 inventories to Captain");

    createLink(deckHost, 801, bookKeeperHost, -1);
    createLink(deckHost, 802, captainHost, -1);

    let data = getBookKeeperData(testRun);
    let currentInventory = []
    let inventories = [currentInventory]
    ...
    let expectedResult = inventories
        .map(function (inv) { return inv.reduce(function (a, b) { return a + b; }, 0); })
        .sort(function (a, b) {return a - b;}).reverse()
        .slice(0, 3)
        .reduce(function (a, b) { return a + b; }, 0);

    let upperDigits = data.map(function (x) {return (typeof x === 'string') ? x : ((x / 1000) | 0);});
    let lowerDigits = data.map(function (x) {return (typeof x === 'string') ? x : x % 1000;});
    let bookKeeperUpperDigitsFile = createLockedFile(bookKeeperHost, 201, FILE_ICON_ARCHIVE, upperDigits);
    let bookKeeperLowerDigitsFile = createLockedFile(bookKeeperHost, 200, FILE_ICON_ARCHIVE, lowerDigits);

    if (testRun > 3) {
        setFileInitiallyCollapsed(bookKeeperLowerDigitsFile);
        setFileInitiallyCollapsed(bookKeeperUpperDigitsFile);
    }

    let separatorCharRegister = createRegister(bookKeeperHost, 14, -1, "SEPA");
    setRegisterReadCallback(separatorCharRegister, function () {
        return "";
    });

    let captainRegister = createRegister(captainHost, 14, 3, "CAPT");

    let expectedLower = expectedResult % 1000;
    let expectedUpper = (expectedResult / 1000) | 0;

    printWindow(captainTerminal, "Awaiting Digits. 0/2");
    printWindow(captainTerminal, "Expecting Digits: (" + expectedUpper + ", " + expectedLower + ")");

    let digitInput = [0, 0];
    let currentDigitIndex = 0;
    setRegisterWriteCallback(captainRegister, function (input) {
        if (currentDigitIndex === 0) {
            digitInput[currentDigitIndex] = input;
            currentDigitIndex++;
            clearWindow(captainTerminal);
            printWindow(captainTerminal, "Awaiting Digits. 1/2");
        } else if (currentDigitIndex === 1) {
            digitInput[currentDigitIndex] = input;
            currentDigitIndex++;

            let firstInputMatch = 0; // use 0 for wrong input, 1 for correct lower, 2 for correct upper
            if (digitInput[0] === expectedLower) {
                firstInputMatch = 1;
            } else if (digitInput[0] === expectedUpper) {
                firstInputMatch = 2;
            }

            let secondInputMatch = 0;
            if (firstInputMatch !== 1 && digitInput[1] === expectedLower) {
                secondInputMatch = 1;
            } else if (firstInputMatch !== 2 && digitInput[1] === expectedUpper) {
                secondInputMatch = 2;
            }

            clearWindow(captainTerminal);
            if (firstInputMatch === 0 && secondInputMatch === 0) {
                printWindow(captainTerminal, "Incorrect upper and lower digits.");
                printWindow(captainTerminal, "Expected (" + expectedUpper + ", " + expectedLower + "). Got (" + digitInput[0] + ", " + digitInput[1] + ")");
                setCustomGoalFailed(captainGoal);
            } else if (firstInputMatch > 0 && secondInputMatch > 0) {
                printWindow(captainTerminal, "That is correct!");
                setCustomGoalCompleted(captainGoal);
            } else if (firstInputMatch === 1 || secondInputMatch === 1) {
                printWindow(captainTerminal, "Incorrect upper digits.");
                let wrongInput = firstInputMatch === 0 ? digitInput[0] : digitInput[1];
                printWindow(captainTerminal, "Expected " + expectedUpper + ". Got " + wrongInput);
                setCustomGoalFailed(captainGoal);
            } else {
                printWindow(captainTerminal, "Incorrect lower digits.");
                let wrongInput = firstInputMatch === 0 ? digitInput[0] : digitInput[1];
                printWindow(captainTerminal, "Expected " + expectedLower + ". Got " + wrongInput);
                setCustomGoalFailed(captainGoal);
            }
        }
    });
