...
function initializeTestRun(testRun) {
    var deckHost = createHost("Deck", 5, -1, 5, 5);
    var bookKeeperHost = createHost("Bookkeeper", 12, -1, 3, 2);
    var captainHost = createHost("Captain", 12, 2, 3, 2);
    var captainTerminal = createWindow("Captain's Terminal", 80, 3, 40, 2);
    var captainGoal = requireCustomGoal("Report sum of max 3 inventories to Captain");

    createLink(deckHost, 801, bookKeeperHost, -1);
    createLink(deckHost, 802, captainHost, -1);

    var data = getBookKeeperData(testRun);
    var currentInventory = []
    var inventories = [currentInventory]
    ...
    var expectedResult = inventories
        .map(function (inv) { return inv.reduce(function (a, b) { return a + b; }, 0); })
        .sort(function (a, b) {return a - b;}).reverse()
        .slice(0, 3)
        .reduce(function (a, b) { return a + b; }, 0);

    var upperDigits = data.map(function (x) {return (typeof x === 'string') ? x : ((x / 1000) | 0);});
    var lowerDigits = data.map(function (x) {return (typeof x === 'string') ? x : x % 1000;});
    var bookKeeperUpperDigitsFile = createLockedFile(bookKeeperHost, 201, FILE_ICON_ARCHIVE, upperDigits);
    var bookKeeperLowerDigitsFile = createLockedFile(bookKeeperHost, 200, FILE_ICON_ARCHIVE, lowerDigits);
    ...
    var separatorCharRegister = createRegister(bookKeeperHost, 14, -1, "SEPA");
    setRegisterReadCallback(separatorCharRegister, function () {
        return "";
    });

    var captainRegister = createRegister(captainHost, 14, 3, "CAPT");

    var expectedLower = expectedResult % 1000;
    var expectedUpper = (expectedResult / 1000) | 0;
    ...
    setRegisterWriteCallback(captainRegister, function (input) {
        if (currentDigitIndex == 0) {
            ...
            var firstInputMatch = 0; // use 0 for wrong input, 1 for correct lower, 2 for correct upper
            ...
            if (firstInputMatch === 0 && secondInputMatch === 0) {
                ...
                var wrongInput = firstInputMatch === 0 ? digitInput[0] : digitInput[1];
...
function getBookKeeperData(testRun) {
    ...
    var numElves = randomInt(20, 100);
    var data = []
    for (var i = 0; i < numElves; i++) {
        if (i > 0) {
            data.push("");
        }
        var numItems = randomInt(1, 20);
        var upperDigitRemaining = 25;
        var lowerDigitRemaining = 9000;
        for (var itemID = 0; itemID < numItems; itemID++) {
            var upperDigitValue = randomInt(1, Math.max(5, upperDigitRemaining));
            var lowerDigitValue = randomInt(1, Math.max(20, Math.min(999, lowerDigitRemaining)));
