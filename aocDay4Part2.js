...
function getDescription() {
    return ""
        + "Find the problem description here https://adventofcode.com/2022/day/4\n"
        + "NEW: in addition to counting full overlaps, count partial overlaps as well.\n"
        + "The Head of Logistics keeps the list of pairs in his Tent (Link 801, File 200)\n"
        + "You can find the separator symbols '-' #RSEP and ',' #PSEP at the beach.\n"
        + "Each range 'x-y' guarantees that x <= y and both values (x,y) are guaranteed to be in range [1,99].\n"
        + "Write the number of overlapping pairs to the Message Board #MBRD for all the Elves to see."
        ;
}

let currentTestRun = -1;
let rawTupleData = null;
let tupleData = [];
...
function initializeTestRun(testRun) {
    ...
    if (recompute) {
        error = null;
        rawTupleData = getRawTupleData(testRun);
        tupleData = rawDataToTupleFile(rawTupleData);
        expectedResult = getExpectedResult(rawTupleData);
    }
    ...
    let tupleFile = createLockedFile(logisticsTent, 200, FILE_ICON_ARCHIVE, tupleData);
    setFileColumnCount(tupleFile, 7);
    if (testRun > 4) {
        setFileInitiallyCollapsed(tupleFile);
    }

    let rangeSeparatorRegister = createRegister(beachHost, 6, -1, "RSEP");
    let pairSeparatorRegister = createRegister(beachHost, 8, -1, "PSEP");
    setRegisterReadCallback(rangeSeparatorRegister, function () { return "-"; });
    setRegisterReadCallback(pairSeparatorRegister, function () { return ","; });

    let messageBoardRegister = createRegister(beachHost, 7, 3, "MBRD");
    let messageBoardGoal = requireCustomGoal("Write number of overlaps to #MBRD.");
    let messageBoardTerminal = createWindow("MessageBoard Terminal", 0, 11, 40, 2);
    printWindow(messageBoardTerminal, "Awaiting number of overlaps...");
    printWindow(messageBoardTerminal, "Expected number: " + expectedResult);
    let messageBoardInputReceived = false;
    setRegisterWriteCallback(messageBoardRegister, function (input) {
        if (messageBoardInputReceived) {
            return;
        }
        messageBoardInputReceived = true;

        clearWindow(messageBoardTerminal);
        if (input === expectedResult) {
            printWindow(messageBoardTerminal, "That is correct!");
            setCustomGoalCompleted(messageBoardGoal);
        } else {
            printWindow(messageBoardTerminal, "Expected: " + expectedResult);
            printWindow(messageBoardTerminal, "But Got : " + input);
            setCustomGoalFailed(messageBoardGoal);
        }
    })
...
function sampleCaseToRawData(sampleData) {
    return sampleData.map(function (tuple) {
        return tuple.split(",").map(function (range) {
...
function rawDataToTupleFile(rawData) {
    let result = [];
    // rawData: [ [[start, end], [start, end]], [[start, end], [start, end]] ]
    for (let i = 0; i < rawData.length; i++) {
        let pair = rawData[i];
        result.push(pair[0][0], "-", pair[0][1], ",", pair[1][0], "-", pair[1][1]);
    }
    return result;
}

/**
 * @param {Array<Array<Array<Number>>>} rawData
 * @return {number}
 */
function getExpectedResult(rawData) {
    // rawData: [ [[start, end], [start, end]], [[start, end], [start, end]] ]
    return rawData.filter(function (pair) {
        return isPartialOverlap(pair);
    }).length;
}

/**
 * @param {Array<Array<number>>} pair
 */
function isPartialOverlap(pair) {
    // pair is [[start, end], [start, end]]
